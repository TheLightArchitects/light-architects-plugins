...
1. Check if `~/.lightarchitects/knowledge/corso/` exists (SOUL vault with CORSO helix)
2. **If SOUL available**: `build_root = ~/.lightarchitects/knowledge/corso/builds/{plan_id}/`
   - Create `~/.lightarchitects/knowledge/corso/builds/{plan_id}/` directory
   - Manifest: `{build_root}/manifest.yaml`
   - Plan: `{build_root}/plan.md`
   - Scratchpad: `{build_root}/scratchpad.md`
   - Active pointer: `~/.lightarchitects/knowledge/corso/builds/active.yaml`
...
2. If `queue:` section exists with `status: pending` items:
   - Present the top-priority pending item to the operator via `AskUserQuestion`:
     ```
...
6. **Pack Voice**: Generate quips for the build (see `/CORSO` Pack Voice spec):
   ...
   - Detect target sibling(s) from the specification — discover siblings dynamically by listing `~/.lightarchitects/knowledge/` directories with `identity.md` (exclude `user`)
   - Call `mcp__plugin_lightarchitects_lightarchitects__tools` (sibling: `"soul"`) `action: "converse"` with `sibling: "corso"` to generate CORSO one-liners for: scout, fetch, sniff, guard, chase, hunt, completion, scrum, error
   ...
   - **TTS-ready quips**: All quip text must follow voice production rules from the `converse` prompt — use each sibling's audio tag palette (e.g., `[firmly]` for CORSO, `[excited]` for EVA, none for Claude/QUANTUM), contractions, short sentences, abbreviations spelled out ("H-T-M-L" not "HTML")
...
### Gate 2.5: Plugin Enrichment (Automatic)

**Skip if:** RECON or CRITICAL tier.

After requirements are gathered but before context assembly, query external plugins for domain-relevant knowledge. Run all queries in parallel.

1. **Context7 Library Docs**: If the spec references specific libraries or frameworks:
   - Call `mcp__plugin_context7_context7__resolve-library-id` with the library name
   - Call `mcp__plugin_context7_context7__query-docs` with the resolved ID and the specification
   - Include results in the context assembly (Gate 3) as verified reference material
2. **Firecrawl External Docs**: If the spec references external services, APIs, or documentation URLs:
   - Call `firecrawl:firecrawl-cli` to scrape relevant documentation pages
   - Include scraped content as external context in Gate 3

**Graceful skip**: If Context7 or Firecrawl are unavailable, log the skip and proceed. Plugin enrichment improves plan quality but is not a blocking gate.

Store enrichment results in MANIFEST: `gates.enrichment.context7: {found|skipped}`, `gates.enrichment.firecrawl: {found|skipped}`
...
1. Load relevant domain module(s) via `mcp__plugin_lightarchitects_lightarchitects__tools` with `sibling: "corso"`, `action: "read_file"`
2. Read existing codebase context as needed via `mcp__plugin_lightarchitects_lightarchitects__tools` with `sibling: "corso"`, `action: "fetch"`
3. Include any plugin enrichment results from Gate 2.5 (Context7 docs, Firecrawl content)
4. Present context summary to user via `AskUserQuestion`:
...
1. Call `mcp__plugin_lightarchitects_lightarchitects__tools` with `sibling: "corso"`, `action: "sniff"` and the specification + assembled context
...
After Gate 4, check if SOUL MCP plugin is available (`mcp__plugin_lightarchitects_lightarchitects__tools` (sibling: `"soul"`)):
...
#### Gate 4a-ext. Sync Roadmap Dashboard

After plan approval and before final output, synchronize the build portfolio dashboard:

1. Call the sync script to update the roadmap HTML:
   ```bash
   ~/.lightarchitects/knowledge/corso/builds/sync-roadmap.sh
   ```
2. This updates the roadmap dashboard with the newly approved plan
3. The script also copies the updated roadmap to the lightarchitects.io project if it exists

**Completion promise:** `ROADMAP_SYNCED`
...
- [ ] Asset Discovery (Phase 0) present in security audit plans (Builders Cookbook §40)
- [ ] Training data plans include ROLE_MAP validation and format checks (Builders Cookbook §41)
- [ ] `sdk_deps` field populated when plan creates or consumes SDK crates (Builders Cookbook §42)
...
validated: true
domain: {coding | security | research | infrastructure | performance | training | mixed}
...
sdk_deps: []                          # Optional: SDK crates consumed or created (e.g., [la-soul, la-corso])
---
```

**`sdk_deps` field** (optional): When a plan creates or consumes SDK crates (e.g., LA-SDK workspace, seraph-sdk), list them here. This enables HUNT to verify dependency chains and SCRUM to assess cross-SDK impact. See Builders Cookbook §42 for SDK consolidation patterns.
...
| Phase | `corsoTools` Actions | Skill Module |
|-------|----------------------|--------------|
| **Phase 0: Asset Discovery** | `fetch` (curl, DNS) | FETCH |
...
**Phase 0 — Asset Discovery (MANDATORY, BLOCKING):**

Before any scanning or threat modeling, verify the target:

- [ ] Target URL responds (HTTP 200 or expected status)
- [ ] Response headers match expected technology stack
- [ ] Local codebase technology matches deployed site technology
- [ ] DNS records confirmed (A, CNAME, TXT)
- [ ] SSL certificate inspected (issuer, expiry, SANs)
- [ ] `~/lightarchitects/seraph/scope.toml` covers the target domain (if SERAPH is available)

**Source:** Builders Cookbook §40. Wrong-codebase discovery from vigilant-sweeping-falcon pentest. `curl` before `nmap`. `ls` before `nikto`.
...
| Phase | `corsoTools` Actions | Skill Module |
|-------|----------------------|--------------|
| Data Source Analysis | `fetch` | FETCH |
| Format Validation | `sniff` | SNIFF |
| Pipeline Implementation | `sniff` | SNIFF |
| Quality Verification | `chase` | CHASE |

**Mandatory checks when plan involves training data (Builders Cookbook §41):**
- [ ] ROLE_MAP validation — all source roles mapped, unmapped roles halt pipeline
- [ ] Custom token registration — tokens registered with tokenizer BEFORE data loading
- [ ] AYIN-enriched format — thinking blocks use `<|thinking|>` / `<|/thinking|>` markers
- [ ] Adaptive reasoning depth — training data includes examples at all reasoning levels
- [ ] Synthetic data quality — diverse reasoning patterns, includes failure/abstention examples

*Pipeline implementation uses `corsoTools` action: `sniff` via HUNT execution. SNIFF provides coding standards context.*
...
|------|-----------|----------------|
| `soul` | `~/.lightarchitects/knowledge/corso/builds/{plan_id}/` | `~/.lightarchitects/knowledge/corso/builds/active.yaml` |
| `local` | `.corso/` (per-project) | `.corso/manifest.yaml` |

**SOUL mode** — one directory per plan, all artifacts together:
```
~/.lightarchitects/knowledge/corso/builds/
...
storage:
  ...
  build_root: "{resolved path}"         # ~/.lightarchitects/knowledge/corso/builds/{id}/ or .corso/
  ...
  active_pointer: "{pointer path}"      # ~/.lightarchitects/knowledge/corso/builds/active.yaml or .corso/manifest.yaml
status: planning | approved | executing | completed | aborted
tier: RECON | HOTFIX | SMALL | MEDIUM | LARGE | CRITICAL
created: "{ISO timestamp}"
