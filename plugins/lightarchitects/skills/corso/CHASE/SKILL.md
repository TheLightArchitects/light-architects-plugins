...
### Step 1.5: Context7 Test Framework Reference (Mandatory)

Before testing and performance analysis, query Context7 for current test framework documentation relevant to the target codebase. This ensures test patterns follow current framework best practices.

1. Identify the test framework from Step 1 (e.g., `tokio::test`, `proptest`, `jest`, `pytest`)
2. Call `mcp__plugin_context7_context7__resolve-library-id` with the test framework name
3. Call `mcp__plugin_context7_context7__query-docs` with the resolved ID and a query like "test patterns", "async testing", "property-based testing", or the specific testing approach needed
4. Include Context7 results as verified test pattern reference for the chase action

**Graceful skip**: If Context7 MCP plugin is unavailable, log the skip and proceed. Context7 enriches test strategy but is not a blocking gate.

### Step 2: Execute with MCP Tools

Use `mcp__plugin_lightarchitects_lightarchitects__tools` with `sibling: "corso"`, `action: "chase"` for performance analysis and monitoring, applying all ops intelligence context and Context7 test framework reference below.
