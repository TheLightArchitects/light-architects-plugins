...
### Step 1.5: Context7 Pattern Reference (Mandatory)

Before code analysis, query Context7 for current framework and library patterns relevant to the target codebase. This grounds the review in verified, version-specific API documentation.

1. Identify the primary language and framework from Step 1 (e.g., Rust + tokio, TypeScript + Next.js)
2. Call `mcp__plugin_context7_context7__resolve-library-id` with the framework/library name
3. Call `mcp__plugin_context7_context7__query-docs` with the resolved ID and a query like "best practices", "error handling patterns", or the specific API surface being reviewed
4. Include Context7 results as verified pattern reference for the code review

**Graceful skip**: If Context7 MCP plugin is unavailable, log the skip and proceed. Context7 enriches pattern detection but is not a blocking gate.

### Step 2: Execute with MCP Tools

Use `mcp__plugin_lightarchitects_lightarchitects__tools` with `sibling: "corso"`, `action: "code_review"` for quality analysis, applying all coding standards, architecture intelligence, Context7 pattern reference, and domain context below.
