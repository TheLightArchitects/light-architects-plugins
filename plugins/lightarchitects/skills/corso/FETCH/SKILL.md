...
Before executing research, query Context7 for real-time library documentation relevant to the research scope. This provides verified, version-specific docs that ground the research.
...
3. Include Context7 results in the research context passed to the fetch action
...
If the research scope involves external documentation, blog posts, architecture comparisons, or URLs:

1. Call Firecrawl to scrape relevant documentation pages for clean, LLM-optimized content
2. Use for "comparison" and "comprehensive" search strategies (see Search Strategy table)
3. Include scraped results alongside Context7 docs as external reference material

**Graceful skip**: If Firecrawl is unavailable or the research is purely internal (codebase-only), skip this step. Firecrawl enriches external research but is not a blocking gate.

### Step 2: Execute with MCP Tools

Use `mcp__plugin_lightarchitects_lightarchitects__tools` with `sibling: "corso"`, `action: "fetch"` for research, knowledge retrieval, and knowledge graph queries, applying all research intelligence context below. Include any Context7 documentation from Step 1.5 and Firecrawl content from Step 1.6 as verified reference material.
