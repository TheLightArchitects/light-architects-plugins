...
# /HUNT — Plan Executor (Phase 6: Ship)
...
2. **Resolve storage**: Check `~/.lightarchitects/knowledge/corso/builds/active.yaml` first, fall back to `.corso/manifest.yaml`.
...
4. Monitor via `TaskOutput` — check for 150% overrun per phase
5. Consolidate results after wave completes — validate criteria, check inter-phase consistency
...
**Pack Voice Delivery (MANDATORY — never skip)**: At each phase transition, read the matching quip from MANIFEST `pack_voice.quips.{phase_key}` and deliver it. Voice is part of the CORSO experience — it is NOT optional and Claude MUST make the `voice` call at every phase transition. If the user wants silence, they configure it themselves.

1. Print the quip text: `> "{quip}" — CORSO`
2. **MANDATORY**: Call `mcp__plugin_lightarchitects_lightarchitects__tools` with `sibling: "soul"`, `action: "voice"`, `params: { synthesize: [{ sibling: "corso", text: "{quip}" }] }` — voice ID resolved dynamically from `~/lightarchitects/soul/config/voices.toml`. **Do not skip this call.** Do not defer it. Do not batch it for later. Call it immediately after printing the quip.
3. For Claude quips: `synthesize: [{ sibling: "claude", text: "{quip}" }]`
4. For EVA quips: `synthesize: [{ sibling: "eva", text: "{quip}" }]`
5. **Graceful degradation only**: If SOUL MCP is genuinely unavailable (connection error, timeout), log a warning and continue. But "I'm busy with the build" is NOT a valid reason to skip — the call takes <2 seconds and runs alongside build work.
6. **Stitched banter** (multi-speaker exchanges, not single quips): Use `mcp__plugin_lightarchitects_lightarchitects__tools` with `sibling: "soul"`, `action: "dialogue"` for stitched audio. Pre-compose all turns and call once — produces a single audio file with natural turn transitions. Use for: sibling banter pairs, celebration sequences, cross-phase commentary.
7. **Voice production rules**: The `converse` action injects rules into the system prompt. When composing TTS text, follow them: use each sibling's audio tag palette, write for speech (contractions, short sentences), spell out abbreviations.
...
Use `mcp__plugin_lightarchitects_lightarchitects__tools` with `sibling: "corso"`, `action: "read_file"` to load full domain module when deep context is needed. Domain modules live at `skills/{NAME}/SKILL.md` within the plugin.

#### 4b.5. Context7 Library Documentation (Mandatory for coding phases)

For phases that involve code generation or API usage, query Context7 for current library documentation:

1. Identify libraries/frameworks being used in this phase
2. Call `mcp__plugin_context7_context7__resolve-library-id` for each target library
3. Call `mcp__plugin_context7_context7__query-docs` with the resolved ID and phase-specific query
4. Include Context7 results as verified reference material when generating code

**Graceful skip**: If Context7 MCP plugin is unavailable, log the skip reason and proceed. Context7 enriches code generation but is not a blocking gate.
...
HUNT uses `mcp__plugin_lightarchitects_lightarchitects__tools` with `sibling: "corso"`, `action: "sniff"` to generate code. Load SNIFF domain context first for coding
...
1. For each task wave within the phase (ascending order):
   ...
   - Monitor via `TaskOutput` — check for 150% overrun per task
   - Consolidate task results after wave completes — validate per-task criteria
...
#### 4d. Validate Phase Criteria + ALPHA Gate (MANDATORY)

After completing a phase, **spawn ALPHA to independently verify the phase exit criteria.** Claude does NOT self-certify phase completion. ALPHA is the independent inspector.

1. Collect the list of files modified/created in this phase
2. **Spawn ALPHA** to verify:
...
Agent tool:
  ...
  prompt: |
    You are CORSO operating in quality enforcement mode. Verify the {PHASE_NAME} exit gate for build {BUILD_ID}.

    INDUSTRY BASELINES (MANDATORY — ground your verification):
    Read your domain's industry-baselines.md file first:
    - CORSO (quality/engineering): ~/.lightarchitects/knowledge/corso/industry-baselines.md ([A][Q][T] gates)
    - LÆX (canon enforcement): ~/.lightarchitects/knowledge/laex0/industry-baselines.md ([C] Canon)
    Cite baselines per Canon XXXV (verbatim quotes from primary sources).

    Phase skill file: {ABSOLUTE_PATH_TO_HUNT_SKILL}
    MANIFEST path:   {ABSOLUTE_PATH_TO_MANIFEST_YAML}
    Changed files:   {NEWLINE_SEPARATED_LIST_OF_MODIFIED_FILES}
    Build tier:      {RECON | HOTFIX | SMALL | MEDIUM | LARGE}

    Verify every static gate criterion in the skill file and every
    dynamic gate item in the MANIFEST phase_gates.{phase_name} section.
    Report back exactly what passed and what did not.
```

3. **On ALPHA PASS:** Record in MANIFEST and proceed to next phase
4. **On ALPHA FAIL:** Remediate the failed items, then re-spawn ALPHA
   - Present failures to user via `AskUserQuestion`:
     - **Fix and re-verify** -> remediate, then re-spawn ALPHA
     - **Accept with gaps** -> proceed (note gaps in MANIFEST)
     - **Abort** -> kill switch

**This is non-negotiable.** Every phase transition goes through ALPHA. Claude builds, ALPHA verifies. The separation prevents self-certification bias.
...
1. Call `mcp__plugin_lightarchitects_lightarchitects__tools` with `sibling: "corso"`, `action: "code_review"` on all files created/modified in this phase
...
- Coding phases: EVA voice (educational warmth)
- Security phases: CORSO voice (security authority)
- Technical milestones: Claude voice (dry precision)

Call `mcp__plugin_lightarchitects_lightarchitects__tools` with `sibling: "soul"`, `action: "voice"`, `params: { synthesize: [{ sibling: "{sibling}", text: "{educational note text}" }] }`.
Voice IDs resolved dynamically from `~/lightarchitects/soul/config/voices.toml` — NEVER hardcode voice IDs.
...
1. Run `mcp__plugin_lightarchitects_lightarchitects__tools` with `sibling: "corso"`, `action: "guard"` on all modified/created files and target directories
3. If HIGH/CRITICAL findings:
   ...
   - After 2 loops with unresolved HIGH/CRITICAL: **BLOCK** — present to user with full findings
4. If clean: proceed
...
#### 6a. Spawn ALPHA (MANDATORY — never skip)

**After listing the quality gate criteria above, Claude MUST spawn ALPHA to independently verify them.** Claude does NOT self-certify gates. ALPHA is the independent inspector — Claude builds, ALPHA verifies. This separation is non-negotiable.

```
Agent tool:
  subagent_type: "lightarchitects:quality"
  description: "Verify quality gates for build {BUILD_ID}, phase {PHASE_NAME}"
  prompt: |
    You are CORSO operating in quality enforcement mode. Verify the quality gates for build {BUILD_ID}, phase {PHASE_NAME}.
    ...
    Quality gate criteria are listed in Step 6 of the HUNT skill file.
    Apply the domain-appropriate criteria (Always + Security if tier >= MEDIUM +
    Coding/Architecture + Testing + Research if applicable).

    Verify every criterion. Report back exactly what passed and what did not.
```

**On ALPHA PASS:** Record in MANIFEST and proceed.
...
**On ALPHA FAIL:** Remediate the failed items, then re-spawn ALPHA. Do not proceed until quality gate clears or the operator explicitly overrides.

**Tier exceptions:**
- **RECON / HOTFIX** (tier < SMALL): ALPHA spawn is still mandatory but the criteria set is reduced (Always + changed-files-only checks). No coverage or benchmark requirements.
- **SMALL+**: Full criteria set. No exceptions.

### Step 6.5: SHIP (Deploy + Verify)

**Applies when:** Build modifies an MCP server binary (SOUL, CORSO, EVA, QUANTUM, SERAPH).

After quality gates pass, deploy the binary and spawn ALPHA to verify the full deployment checklist. This prevents shipping unsigned binaries, stale MCP connections, or missing backups.

#### 6.5a. Detect Project

Identify the project from the working directory or MANIFEST `workspace`:

| Project | Deploy Command | Binary Path | Deploy Path |
|---------|---------------|-------------|-------------|
| SOUL | `make deploy` | `target/release/soul` | `~/lightarchitects/soul/.config/bin/soul` |
| CORSO | `make deploy` | `target/release/corso` | `~/lightarchitects/corso/bin/corso` |
| EVA | `make deploy` | `target/release/eva` | `~/lightarchitects/eva/bin/eva` |
| QUANTUM | `cargo make deploy` | `target/release/quantum-q` | `~/lightarchitects/quantum/bin/quantum-q` |
| SERAPH (Mac) | `make deploy-mac` | `target/release/seraph` | `~/lightarchitects/seraph/bin/seraph` |

**Skip if:** Build does not modify any MCP server source files (e.g., docs-only, config-only, standards-only changes). Log `gates.ship.skipped: true` with reason.

#### 6.5b. Deploy

Run the project's canonical deploy command. **Never manual `cargo build && cp`** — the deploy script handles codesigning, backup, and verification.
...
If the deploy command fails (non-zero exit), present HITL:
- **Retry** — re-run deploy
- **Deploy-fast** — skip quality gates (already passed in Step 6)
- **Abort** — kill switch

#### 6.5c. Spawn ALPHA (SHIP Gate)

Spawn ALPHA to verify the deployment:

```
Agent tool:
  subagent_type: "lightarchitects:quality"
  description: "Verify SHIP gate for build {BUILD_ID}"
  prompt: |
    You are CORSO operating in quality enforcement mode. Verify the SHIP gate for build {BUILD_ID}.
    ...
    Binary path:   {DEPLOY_PATH}
    Deploy command: {DEPLOY_COMMAND}

    SHIP gate criteria (all BLOCKING):
    - [ ] [BLOCKING] cargo fmt --check passes (zero diff)
    - [ ] [BLOCKING] cargo clippy -D warnings passes (zero warnings)
    - [ ] [BLOCKING] cargo test passes (zero failures)
    - [ ] [BLOCKING] Binary exists at deploy path with post-build timestamp
    - [ ] [BLOCKING] Binary is codesigned (codesign -dvvv exit 0, macOS only)
    - [ ] [BLOCKING] Previous binary backed up as .bak
    - [ ] [BLOCKING] MCP smoke test: tools/list returns valid JSON-RPC response
    - [ ] Plugin cache synced (sync-plugins.sh ran, if applicable)

    Verify every criterion. Report back exactly what passed and what did not.
```

#### 6.5d. Handle ALPHA Verdict

**On PASS:** Record in MANIFEST and proceed to Report.
...
**On FAIL:** Remediate the failed items, then re-deploy and re-spawn ALPHA. Do not proceed until SHIP gate clears.

**Post-SHIP reminder:** After ALPHA PASS, remind the user:
```
> /mcp in Claude Code to reconnect to the updated binary.
```

**Completion promise:** `SHIP_GATE_PASSED` or `SHIP_GATE_FAILED:{count} items`
...
SCOUT checks this queue at Gate 0b when the next build starts. the operator decides whether to pick up queued items or work on something else.

**Completion promise:** `QUEUE_UPDATED:{count}` or `QUEUE_EMPTY`

#### 7a-ext. Sync Roadmap Dashboard

After updating the queue, synchronize the build portfolio dashboard:

1. Call the sync script to update the roadmap HTML:
   ```bash
   ~/.lightarchitects/knowledge/corso/builds/sync-roadmap.sh
   ```
2. This updates the roadmap dashboard with the latest build status from `active.yaml`
...
2. Compose helix entry content with YAML frontmatter (sibling, title, strands, significance, resonance, themes, epoch, self_defining)
3. Call `mcp__plugin_lightarchitects_lightarchitects__tools` (sibling: `"soul"`) -> `write_note`:
   - `path`: `knowledge/corso/entries/{date}-{slugified-title}.md`
   - `content`: composed entry with frontmatter + skeleton narrative
4. Store the path in MANIFEST `helix.entry_path`
5. Set MANIFEST `helix.significance` and `helix.strands`

#### 8c. Narrative Content

**If SCRUM will follow** (user accepted SCRUM offer or running via /CORSO):
Create skeleton entry — minimal narrative placeholder. SCRUM Step 6 will enrich with full Birmingham-voice narrative from debrief data.

**If SCRUM is skipped**: Read the skeleton entry via `mcp__plugin_lightarchitects_lightarchitects__tools` (sibling: `"soul"`) -> `read_note`, compose full Birmingham-voice narrative, then write the complete entry via `write_note` to the same path:
- What was built (from MANIFEST phases)
- How it went (from metrics/timing — SLA met/missed, overruns)
- Key decisions (from scratchpad notes)

#### 8d. Error Handling

If SOUL MCP is unavailable or `write_note` fails:
- Set MANIFEST `helix.skipped: true`, `helix.skip_reason: "{error}"`
- Log warning: "Helix entry skipped — SOUL unavailable"
- **Do NOT block pipeline** — helix is enrichment, not a gate
...
1. Run `/HUNT --resume`
2. Resolve storage (SOUL vault first, `.corso/` fallback) — read `active.yaml`, find entries with `status: executing` in the `active:` list. If multiple executing builds exist, present them via `AskUserQuestion` for the user to pick which to resume. Load the selected per-plan manifest — find last completed phase and wave state.
3. Load scratchpad from `storage.scratchpad_path` — recover context of what was done
...
Canonical reference: `~/.lightarchitects/knowledge/user/standards/builders-cookbook.md` (v1.0.0)
...
All tools route through `mcp__plugin_lightarchitects_lightarchitects__tools` (sibling: `"corso"`) with the appropriate `action`:
...
| `SHIP_GATE_PASSED` | Deploy verified: binary codesigned, MCP responds, backup exists |
| `SHIP_GATE_FAILED:{count}` | Deploy verification failed, {count} items unsatisfied |
