...
| Gatekeeper Registry | `standards/gatekeeper-registry.yaml` | Finding → sibling ownership routing |
...
# Q2 exception (LÆX #44): if failures appear in files NOT in `git diff <base>..HEAD --name-only`
# (pre-existing, unrelated to this branch), scope to --lib --bins and document each file:
# cargo clippy --workspace --lib --bins -- -D warnings  →  record in gate-eval YAML pre_existing_issues_documented[]
...
If LÆX MCP unavailable and S3 is required: **block**. Surface to the operator — no waiver.
...
| LÆX MCP unavailable + S3 triggered | Block. Escalate to the operator — no waiver |
